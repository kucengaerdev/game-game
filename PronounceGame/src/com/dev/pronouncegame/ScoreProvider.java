...
public class ScoreProvider extends ContentProvider {
	...
	public static final Uri CONTENT_URI = Uri.parse("content://com.dev.pronouncegame.scoredatabaseprovider/element");
	...
	static{
		...
		uriMatcher.addURI("com.dev.pronouncegame.scoredatabaseprovider","element",ALLROWS);
		uriMatcher.addURI("com.dev.pronouncegame.socredatabaseprovider","element/name/*", NAME);
		uriMatcher.addURI("com.dev.pronouncegame.scoredatabaseprovider", "element/names", NAME_TO_DELETE);
		uriMatcher.addURI("com.dev.pronouncegame.scoredatabaseprovider","element/filter/*", NAME_FILTER);
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (UriType){
		    case ALLROWS:
			    rowsDeleted = db.delete(gamedb.TABLE_SCORE,selection,selectionArgs);
			    break;
		    case NAME_TO_DELETE:
			    rowsDeleted = db.delete(gamedb.TABLE_SCORE,gamedb.NAMA+"= '"+selection+"'", selectionArgs);
			    break;
	 	    default :
	...
	public Uri insert(Uri uri, ContentValues values) {
		// TODOs Auto-generated method stub
		Log.d("ScoreProvider","firsttime");
		int uritype = uriMatcher.match(uri);
		SQLiteDatabase db = gamedb.getWritableDatabase();
		long id = 0;
		switch(uritype){
		    case ALLROWS:
		    	id = db.insert(gamedb.TABLE_SCORE, null, values);
		        break;
		    default :
		    	throw new IllegalArgumentException("unKnown uri : "+uri);
		}
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
			String sortOrder) {
		// TODOs Auto-generated method stub
		Log.d("ScoreProvider",uri.toString());
		SQLiteQueryBuilder queryB = new SQLiteQueryBuilder();
		checkColumns(projection);
		queryB.setTables(gamedb.TABLE_SCORE);
		int uriType = uriMatcher.match(uri);
		Log.d("ScoreProvider", "uriType "+uriType);
		switch (uriType) {
		case ALLROWS:
		...
		case NAME:
			...
			queryB.appendWhere(gamedb.NAMA+"= '"+uri.getLastPathSegment()+"'");
			break;
		
		case NAME_FILTER:
			
			queryB.appendWhere(gamedb.NAMA+" LIKE '%"+uri.getLastPathSegment()+"%'");
			break;
	...
	private void checkColumns(String[] projection) {
		// TODOs Auto-generated method stub
		String[] avail = {gamedb.NAMA,gamedb.NILAI};
		if (projection != null){
